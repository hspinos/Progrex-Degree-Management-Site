...
function DocumentCard(props) {
	...
	function handleCardClick() {
		if (isSigned) {
			...
			document.getElementById("signedModalFooter").textContent =
				"Date Signed: " + new Date(dateSigned).toLocaleDateString();
		} else {
	...
	if (isSigned) {
		modalFooter = (
			<span className="inline-block bg-green-400 rounded-full px-3 py-1 text-sm font-semibold mr-2 mb-2">
				Completed
			</span>
		);
		modal = "#signedModal";
		...
		modalFooter = (
			<span className="inline-block bg-red-500 rounded-full px-3 py-1 text-sm font-semibold mr-2 mb-2">
				Action Needed
			</span>
		);
		modal = "#unsignedModal";
	...
	function openPowerForm() {
		...
		let queriedUrl =
			powerFormUrl + signerQuery + envelopeQuery + `&fname=${user.fName},`;
	...
	return (
		<div
			...
			className="max-w-sm rounded overflow-hidden shadow-lg rounded-md bg-stone-800 hover:bg-stone-700 hover:cursor-pointer hover:scale-105 transition ease-in duration-200"
		>
			<div className="px-6 py-4">
				<div className="font-bold text-xl mb-2">{props.name}</div>
				<p className="text-white text-base">{props.desc}</p>
			</div>
